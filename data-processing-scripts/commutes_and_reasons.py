...
organized_data = defaultdict(lambda: defaultdict(lambda: defaultdict(
...
location_dict = {**restaurant_dict, **pub_dict, **employer_dict, **apartment_dict}
...
with open(os.getenv('TRAVEL_JOURNAL_DATA_PATH'), newline='') as csvfile:
    ...
    for row in reader:
        start_time = datetime.fromisoformat(row['travelStartTime'][:-1])  # Parse the start time
        ...
        portion_of_day = 'Morning' if start_time.hour < 12 else ('Afternoon' if start_time.hour < 14 else ('Evening' if start_time.hour < 18 else 'Night'))
        purpose = row['purpose']  # Get the purpose of the commute
        start_building_id = row['travelStartLocationId']  # Get the start building ID
        ...
        # Increment the commute count for the specific day, portion of the day, month, purpose, start and end location types
        organized_data[month][day_of_week][portion_of_day][purpose][start_location_type][end_location_type] += 1

# Write the organized data to a CSV file
with open('../data/commutes_and_reasons.csv', 'w', newline='') as csvfile:
    fieldnames = ['Month', 'Day of Week', 'Portion of Day', 'Purpose', 'Start Location Type', 'End Location Type', 'Total Commutes']
    ...
    for month, day_data in organized_data.items():
        for day_of_week, portion_data in day_data.items():
            for portion_of_day, purpose_data in portion_data.items():
                for purpose, start_location_data in purpose_data.items():
                    for start_location_type, end_location_data in start_location_data.items():
                        for end_location_type, total_commutes in end_location_data.items():
                            writer.writerow({
                                ...
                                'Purpose': purpose,
                                'Start Location Type': start_location_type,
                                'End Location Type': end_location_type,
                                'Total Commutes': total_commutes
                            })

print('CSV file has been created: commutes_and_reasons.csv')